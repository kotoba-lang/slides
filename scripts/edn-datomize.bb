...
;; kotoba-lang/slides 用に com-junkawasaki/root superproject の
;; manifest/edn-datomize.bb から移植（Phase 3 fanout。gftdcojp/net-kotobase の
;; Phase 2 pilot 版 — wrap-generic モード付き — をベースにした）。
...
;;   bb scripts/edn-datomize.bb wrap-generic <path> <default-ns> — 既存名前空間キーを保持して変換
...
;; kotoba-lang/slides (child repo) has no manifest/ dir — schema lives at repo root.
...
;; ---------- generic (top-level map with keys already namespaced) ----------
;;
;; Unlike wrap-map! (which forces every key under one new ns-name), wrap-generic!
;; keeps a key's existing namespace when it already has one and only assigns
;; default-ns to genuinely bare keys. This avoids renaming attributes that
;; downstream code destructures by their original (already-meaningful)
;; namespaced key.
...
;; ---------- ADR-style dirs (not used in this repo currently, kept for parity) ----------
