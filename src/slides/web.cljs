...
(defn shape-node [idx shape]
  (let [d (deck)
        shape (design/resolve-shape d shape)
...
(defn select-options [values selected]
  (apply str
         ...
         (for [value values
               :let [s (name value)]]
           (str "<option value=\"" s "\"" (when (= value selected) " selected") ">" s "</option>"))))

(defn selected-properties []
  (if-let [raw-shape (selected-shape)]
    (let [shape (design/resolve-shape (deck) raw-shape)]
      (str "<div class=\"panel-title\">Shape</div>"
         (property-input "ID" "shape-id" (:slides/id raw-shape (:slides/id shape "")) "")
         "<label><span>Component</span><select id=\"shape-component\">"
         (select-options (keys (:slides/components (design/deck-design (deck)))) (:slides/component raw-shape))
         "</select></label>"
         "<label><span>Text style</span><select id=\"shape-text-style\">"
         (select-options (keys (:slides/text-styles (design/deck-design (deck)))) (:slides/text-style raw-shape))
         "</select></label>"
      ...
      (replace-shape
       ...
       (fn [shape]
         (let [kind (keyword (.-value ($ "shape-kind")))
               component-value (.-value ($ "shape-component"))
               style-value (.-value ($ "shape-text-style"))
               base (assoc shape
                           :slides/id (.-value ($ "shape-id"))
                           :slides/shape kind
                           :slides/component (when-not (str/blank? component-value) (keyword component-value))
                           :slides/text-style (when-not (str/blank? style-value) (keyword style-value))
...
(defn add-component! [component-id]
  (let [idx (selected-slide-index)
        shapes (:slides/shapes (selected-slide))]
    (replace-slide idx #(update % :slides/shapes conj
                                {:slides/id (str (name component-id) "-" (inc (count shapes)))
...
(defn import-pptx! [file]
  (let [reader (js/FileReader.)]
    (set! (.-onload reader)
          (fn [event]
            (-> (read-pptx-entries (.. event -target -result))
                (.then
                 (fn [entries]
                   (let [parsed (pptx-entries->deck entries (.-name file))]
                     ...
                     (fn [event]
                       (let [target (.-target event)]
                         (cond
                           ...
                           (.closest target "#add-title") (add-component! :title)
                           (.closest target "#add-panel") (add-component! :panel)
