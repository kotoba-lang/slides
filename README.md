...
the static CSS resource. The shell is static SSR from the CLJC view model, and
the SAME view model is mounted live in the browser by the checked-in hydration
bundle `docs/js/main.js` (`slides.web.client`, shadow-cljs `:advanced` release —
the reference implementation of the shitsuke dual-render contract's browser
...
GitHub Pages publishes the SSR shell generated by this library plus the
browser hydration bundle (`slides.web.client` → `docs/js/main.js`): the page
loads as static SSR and becomes the live editor on mount — shape editing
(click/drag/resize/arrow-nudge), fast text editing, undo/redo, zoom, EDN
round-trip, and EDN import/download all run in-browser. Local persistence is
intentionally absent (reload restores the sample deck), and PPTX
import/export stays on the JVM CLI (the zip host is `#?(:clj …)`):
...
| Text/paragraphs | Paragraph right-to-left direction (`rtl`) | ✅ full regen | added this session — `:rtl` alone (no other paragraph-level property) still writes a `<a:pPr rtl="1">`, matching the same fix already applied to run-level properties; full regen only, same as every other paragraph-level property (the patch path always copies `<a:pPr>` through untouched by design) |
...
| Text/paragraphs | Underline style beyond single (double/heavy/wavy/etc) + underline's own color | ✅ full regen + patch | added this session — `:slides/underline-style` alone (no separate `:slides/underline true` needed) implies the underline is on; `set-rpr-underline-color`, same shape as `set-rpr-highlight` |
...
| Table | Cell borders (straight + diagonal), margins, vertical anchor | ✅ full regen | each side's own dash style (`<a:prstDash>`) added this session — previously only width/color were written, so a dashed cell border always regenerated solid |
...
| Patch/update path | Run formatting (bold/italic/underline/strikethrough/baseline/char-spacing/highlight) | ✅ | italic/underline/strikethrough/baseline added this session, fixing a real bug — `apply-rpr-overrides`/`patch-text`'s gate previously only recognized font-size/bold/color/char-spacing/highlight/hyperlink, so an edit touching ONLY one of these four keys (no accompanying text/font-size/color change) was silently dropped entirely; all attribute-only overrides (`set-open-tag-attr`, same shape as bold's own `b=`) |
| Patch/update path | Hyperlink: built-in navigation action (`ppaction://...`) | ✅ | `set-hlink-action`, added this session — inserted right before `</a:rPr>`, wired into `apply-rpr-overrides` so it applies whether or not the run's text also changes in the same edit |
| Patch/update path | Hyperlink: external URL / internal slide jump | ✅ | `patch-hyperlink-relationships`/`set-hlink-rid`, added this session — allocates a fresh rId continuing past the slide's own `.rels` existing max (same convention `patch-new-content` uses for brand-new content), writes the matching External/Internal relationship, and edits the run's own `<a:hlinkClick>`; always allocates fresh rather than reusing/rewriting an existing relationship when only the target changed, leaving any prior entry orphaned but harmless |
| Patch/update path | Lock flags (picture/shape/table-or-chart) | ✅ | `patch-pic-locks`/`patch-graphic-frame-locks`/`patch-sp-locks`, added this session — the first two replace attributes in place (the host element always has exactly one lock child in this writer's own output); `patch-sp-locks` rebuilds the whole `<p:cNvSpPr>` (self-closing or paired) since a shape can legitimately have no lock child at all |
| Patch/update path | Body properties (wrap/anchor/margins/autofit/vertical text direction) | ✅ | `patch-body-props`, added this session — regenerates the whole `<a:bodyPr>` via the same `body-pr-xml` full regen uses (these all live on one element, same rationale as `patch-effects`) |
| Patch/update path | Table style flags, column widths/row heights | ✅ | `patch-table-style-flags`/`patch-table-column-widths`/`patch-table-row-heights`, added this session — the dimension patches walk each `<a:gridCol>`/`<a:tr>` in document order via `str/replace`'s function-replacement form (Matcher-based, so repeated identical widths/heights don't all collapse onto the first match); only applied when the list length matches the table's actual column/row count |
| Patch/update path | Per-slide background | ✅ | `patch-slide-background`, added this session — operates on the whole slide xml, not through `patch-shape-block`, since `<p:bg>` lives on `<p:cSld>`, not inside any one shape; an explicit `nil` override removes an existing `<p:bg>` entirely (reverting to layout/master inheritance) rather than resolving to the master's own color |
| Patch/update path | Already-imported speaker notes text edited *in place* | ✅ | `patch-existing-notes`, added this session — resolves the notesSlide part via the slide's own `.rels` (the deck model carries no direct locator for it), then reuses `patch-paragraphs` (the same logic already used for a shape's own `<p:txBody>`) against the notesSlide's own text |
| Patch/update path | Already-imported comments edited/added/removed in place | ✅ | `patch-existing-comments`, added this session — regenerates the whole comments part from `:slides/comments` (added/removed/reordered/edited alike, one code path); new author names get a fresh id appended to the shared `commentAuthors.xml`, existing authors (including ones only OTHER slides' comments reference) keep their id — corrects a prior claim here: `patch-new-content` never actually handled comments at all, not even brand-new ones |
| Patch/update path | Brand-new comments part (a slide with no comments before this edit) | ✅ | `patch-new-content`, extended this session — new part + relationship + content-type overrides, and (only when the deck had no comments anywhere yet) a brand-new `commentAuthors.xml` wired into `ppt/_rels/presentation.xml.rels`; this session's own patch also fixed a latent gap it surfaced — a slide needing ONLY a first-time notes/comments addition, with no accompanying brand-new shape, used to be skipped entirely (the existing notes test had been silently working around it with a dummy trigger shape) |
