...
| Patch/update path | Per-slide background | ❌ full-regen-only | this feature exists in the full-regen writer (see rows above) but `patch-shape-block` doesn't yet mirror it — editing an imported deck via `update` silently drops any change to this, even though a from-scratch `pptx-bytes` build honors it correctly |
