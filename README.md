...
| Patch/update path | Already-imported comments text edited *in place* | ❌ not implemented | a materially larger feature than notes — multiple per-slide entries in a shared, package-wide `commentAuthors.xml`, each needing its own identity/matching; brand-new comments on a previously-comment-less slide are still added fine by `patch-new-content` |
