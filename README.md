...
| Patch/update path | Hyperlink: external URL / internal slide jump | ❌ full-regen-only | needs a brand-new package relationship (`.rels` + possibly `[Content_Types].xml`), a bigger lift than an in-place `<a:rPr>` edit — out of scope for the built-in-action patch above |
